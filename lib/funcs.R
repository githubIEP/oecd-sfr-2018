cluster = function(pca, num.clusters = 8, cluster.method = "ward.D2", round.numbers = T){
...
extend.time.series = function(df, replace.with = NULL){
    ...
    df = pad(df, start_val = min(df$date),
             end_val = max(df$date), group = c("iso3c", "variablename"), interval = "year")
...
interpolate = function(df){
    ...
    if(diff(range(df$year))>0){
        num.years.test = df %>% group_by(iso3c, variablename) %>%
        ...
        #interpolate
        df <- df %>% group_by(iso3c, variablename) %>% do(interpolate.data(.))
        df = as.data.frame(df)
    }else{
 ...
 counter = 1
  ...
  { 
    return(lapply(y, function(x)
    {
      ...
      direction = direction %>% group_by(iso3c) %>% dplyr::summarise(av = mean(score), worst.case = max(score))
...
impute = function(raw.data, use.precomupted = F)
{
  ############
...
{
  x = merge(x, read.csv("./data/population data/world_pop_2017.csv"), all.x=T)
  x$value = 100000*x$value/x$pop2017
  x = x %>% dplyr::select(-pop2017)
}
